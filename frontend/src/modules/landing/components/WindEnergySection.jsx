...
export default function WindEnergySection() {
  ...
  const featureCards = [
    {
      title: t('wind.cards.solutionsTitle'),
      desc: t('wind.cards.solutionsDesc'),
    },
    {
      title: t('wind.cards.epcTitle'),
      desc: t('wind.cards.epcDesc'),
    },
    {
      title: t('wind.cards.gridTitle'),
      desc: t('wind.cards.gridDesc'),
    },
    {
      title: t('wind.cards.costsTitle'),
      desc: t('wind.cards.costsDesc'),
    },
  ];

  return (
    <section id="wind-energy" className="py-24 lg:py-32 bg-white relative overflow-hidden">
      {/* Decorative Blur Backgrounds */}
      <div className="absolute top-1/4 left-[-10%] w-[350px] h-[350px] rounded-full bg-blue-50/50 blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-[-10%] w-[350px] h-[350px] rounded-full bg-amber-500/5 blur-[120px] pointer-events-none" />
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col justify-center">
            ...
            >
              <div className="flex items-center space-x-3 text-slate-700">
                ...
                <span className="text-xs sm:text-sm font-bold font-heading">{t('wind.stats.clean')}</span>
              </div>
              <div className="flex items-center space-x-3 text-slate-700">
                <div className="p-2 bg-blue-50 rounded-lg text-[#0A2463]">
                  <Globe className="w-4 h-4" />
                </div>
                <span className="text-xs sm:text-sm font-bold font-heading">{t('wind.stats.carbon')}</span>
              </div>
              <div className="flex items-center space-x-3 text-slate-700">
                <div className="p-2 bg-blue-50 rounded-lg text-[#0A2463]">
                  <Settings className="w-4 h-4" />
                </div>
                <span className="text-xs sm:text-sm font-bold font-heading">{t('wind.stats.epc')}</span>
              </div>
              <div className="flex items-center space-x-3 text-slate-700">
                <div className="p-2 bg-blue-50 rounded-lg text-[#0A2463]">
                  <TrendingUp className="w-4 h-4" />
                </div>
                <span className="text-xs sm:text-sm font-bold font-heading">{t('wind.stats.savings')}</span>
              </div>
            </motion.div>
          ...
          {/* Staggered Visual & Cards Block (Right Column) */}
          <div className="lg:col-span-6 flex flex-col gap-8">
            
            {/* Visual turbine panel */}
            <motion.div
