    ...
    <footer className="bg-[#0F172A] border-t border-slate-800 text-slate-400 py-16 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 border-b border-slate-800 pb-12">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <p className="text-xs text-slate-400 max-w-sm leading-relaxed">
              {settings.footerDescription || "turnkey project consultants and financial facilitators enabling manufacturing excellence across Gujarat, Maharashtra, Madhya Pradesh & Rajasthan."}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs leading-relaxed text-slate-400">
              Hotline: {settings.contact?.phone || "+91 99786 55799"} <br />
              Email: {settings.contact?.email || "info@worldexportbhc.com"} <br />
        ...
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs">
          <div>
            &copy; {new Date().getFullYear()} {settings.companyName || "World Export Business Housing Centre"}. All Rights Reserved.
          </div>
          
          {/* Social icons */}
          <div className="flex items-center space-x-4">
            {settings.socialLinks?.facebook && (
              <a href={settings.socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-900 rounded-full text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition-all">
            ...
            {settings.socialLinks?.twitter && (
              <a href={settings.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-900 rounded-full text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition-all">
                <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
            ...
            {settings.socialLinks?.linkedin && (
              <a href={settings.socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-900 rounded-full text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition-all">
                <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
